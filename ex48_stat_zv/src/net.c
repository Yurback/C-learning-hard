...
int attempt_listen(struct addrinfo *info)
{
    ...
    check_debug(rc == 0, "Failed to set SO_REUSADD");
...
{
    int rc = 0;
    int sockfd = -1; // default fall value
    ...
    check(rc == 0, "Failed to get addres info for connect");

    // cycle though the available list to find one
    ...
    // either we found one and were able to listen
