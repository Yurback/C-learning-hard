#include "minunit.h"
#include <stralgo/string_algos.c>
#include <lcthw/bstrlib.h>
#include <time.h>

struct tagbstring IN_STR = bsStatic("I have ALPvHA beta ALPvHA and oranges ALPHA");
...
char *test_strstr()
{
    ...
    do
    {
        for (i = 0; i < 1000; i++)
        {
            found_at = strstr((unsigned char *)IN_STR.data, (unsigned char *)ALPHA.data);
            mu_assert(found_at != NULL, "Failed to find!");
            find_count++;
        }
    ...
    debug("STRSTR COUNT: %lu, END TIME: %d, OPS: %f",
          find_count, (int)elapsed, (double)find_count / elapsed);
    return NULL;
}
