...
#include "hashmap_algos/hashmap_algos.h"
...
int gen_keys(DArray *keys, int num_keys)
{
    ...
    for (i = 0; i < num_keys; i++)
    {
        ...
        // {
        //     PRINT_s(bdata(key));
        //     PRINT_int(blength(key));
        //     PRINT_int(keys->expand_rate);
        // }
...
char *test_distribution()
{
    ...
    fill_distribution(stats[DEFHASH], keys, Default_hash);
    ...
    for (i = 0; i < BUCKETS; i++)
    {
        fprintf(stderr, "%d\t%d\t%d\t%d\n",
                stats[ALGO_FNV1A][i],
                stats[ALGO_ADLER32][i], stats[ALGO_DJB][i], stats[DEFHASH][i]);
    }
